...
/**
 * This is a class of escape room. we can use this class to implement the program of escape room By
 * dragging and drop things to find the clue to turn on the computer and then escape the room.
 * 
 * @author yijun cheng & yuedong cui
 *
 */
public class EscapeRoom extends PApplet {
  private PImage backgroundImage;// this is a field to store the image of background
  private ArrayList<Thing> allThings;// An arrayList to store all of the things.

  /**
   * setting the size of the escape room.
   */
  ...
  /**
   * setup method. Initialize the escape room. And load the image of the back ground and other
   * things, actions and so on used for the draw method.
   */
  public void setup() {
  ...
  /**
   * this method can be used to draw the image of background and things. And add things to the
   * allThings ArrayList. Then remove things that are not active.
   */
  public void draw() {
    image(backgroundImage, 0, 0);//draw the image of the background
    for (int i = 0; i < allThings.size(); i++) {
      Action action = allThings.get(i).update();//get the action of that thing
      if (action != null) {
        action.act(allThings);//do the action of that thing
      }
      if (!allThings.get(i).isActive()) {//if the thing is not active, remove it
        allThings.remove(i);
        i = i - 1;//for regular loop
      }
    }
  ...
  /**
   * this is the main method which is used for user interface to run the escape program.
   * @param args the array of all user input with String type.
   */
  public static void main(String[] args) {
    PApplet.main("EscapeRoom");//create a object of EscapeRoom. 
  }
}

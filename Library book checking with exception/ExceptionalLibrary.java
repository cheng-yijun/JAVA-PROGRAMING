 ...
 *
 * @author yijuncheng & yueddong cui
 */
...
public class ExceptionalLibrary {
  ...
  public void addBook(String title, String author) {
    books.add(new Book(title, author));// add the new book to books ArrayList
  ...
  public void checkCommandArgumentsCount(String[] commands, int validArgCount)
    ...
    if (commands.length != validArgCount)// compare the number of commands inputed and the number of
                                         // commands expected
  ...
  public int parsePinCode(String s, int errorOffset) throws ParseException {
    try {
      ...
      if (pin < 1000 || pin > 9999) {// the pin code must be 4 digits number
        throw new ParseException(
            "Syntax Error: Invalid PIN. The PIN MUST be a 4-digits "
            + "number greater than or equal to 1000.\n",
            errorOffset);
      }
      return pin;
    } catch (NumberFormatException e) {// if the pin is not number format
      throw new ParseException(
          "Syntax Error: Invalid PIN. The PIN MUST be a 4-digits "
          + "number greater than or equal to 1000.\n",
          errorOffset);
  ...
  public int parseCardBarCode(String s, int errorOffset) throws ParseException {
    try {
      int cardBarCode = Integer.parseInt(s);// try to convert card bar code to number format
      if (!Subscriber.checkCardBarCode(cardBarCode)) {// check if the card bar code has 10 digits
        throw new ParseException(
            "Syntax Error: The Subscriber card bar code MUST be a 10-digits number.\n",
            errorOffset);
      }
      return cardBarCode;
    } catch (NumberFormatException e) {// if the card bar number is not a number format
      throw new ParseException(
          "Syntax Error: The Subscriber card bar code MUST be a 10-digits number.\n", errorOffset);
    }
  }
  ...
  public int parseBookId(String s, int errorOffset) throws ParseException {
    try {
      int bookId = Integer.parseInt(s);// try to convert book ID to number format
      return bookId;
    } catch (NumberFormatException e) {// if the book ID is not a number format.
  ...
  public void parseRunLibrarianAddSubscriberCommand(String[] commands)
    ...
    int pin = this.parsePinCode(commands[2], 2);// parse the pin code with a location 2 in commands
    this.parsePhoneNumber(commands[4], 4);// parse the phone number with a location 4 in commands
  ...
  public void parseRunLibrarianCheckoutBookCommand(String[] commands) throws ParseException {
    ...
    this.checkCommandArgumentsCount(commands, 3);// checks if commands stores 3 arguments
    int bookId = this.parseBookId(commands[2], 2);// parse the book ID with a location 2 in commands
    int cardBarCode = this.parseCardBarCode(commands[1], 1);// parse the card bar code with a
                                                            // location 4 in commands
  ...
  public void parseRunLibrarianRemoveBookCommand(String[] commands) throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);// checks if commands stores 2 arguments
    int bookId = this.parseBookId(commands[1], 1);// parse the book ID with a location 1 in commands
  ...
  public void parseRunSubscriberReturnBookCommand(String[] commands, Subscriber subscriber)
    ...
    Book book = this.findBook(bookId);
    if (book != null) {
      if (subscriber.isBookInBooksCheckedOut(book)) {
        subscriber.returnBook(book);// if this book is checked for this subscriber.
      } else
        System.out.println("Error, this book has not been ckecked for you");
    }
  }
  ...
  public void parseRunSubscriberUpdatePhoneNumberCommand(String[] commands, Subscriber subscriber)
      throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);// checks if commands stores 2 arguments
    this.parsePhoneNumber(commands[1], 1);// parse the phone number with a location 1 in commands
    subscriber.setPhoneNumber(commands[1]);// update the phone number
  ...
  public void parseRunSubscriberUpdateAddressCommand(String[] commands, Subscriber subscriber)
    ...
    this.checkCommandArgumentsCount(commands, 2);// checks if commands stores 2 arguments
    subscriber.setAddress(commands[1]);// update the address.
  ...
  public void parseRunLibrarianSaveBooksCommand(String[] commands) throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);// checks if commands stores 2 arguments
    String filename = commands[1];// get the filename from user
    File file = new File(filename);
    try {
      PrintWriter fout = new PrintWriter(file);
      for (int i = 0; i < books.size(); i++) {// write to the file
        fout.println(books.get(i).getTitle() + ":" + books.get(i).getAuthor());
      }
      fout.close();
    } catch (FileNotFoundException e) {
      System.out.print("Error: Could NOT save books contents to file " + commands[1]);
    }
  }
  ...
  public void parseRunLibrarianLoadBooksCommand(String[] commands) throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);// checks if commands stores 2 arguments
    String filename = "";
    filename = commands[1];// get the filename from the user
    File file = new File(filename);
    try {
      Scanner fin = new Scanner(file);
      while (fin.hasNextLine()) {// read the file
        String line = fin.nextLine();
        if (!line.contains(":")) {
          // System.out.println("Found incorrectly formatted line in file books3.txt: " + line);
          continue;
        }
        String[] bookTraits = line.split(":");
        if (bookTraits.length != 2) {// need title and author correctly
          System.out.println(
              "Error: Found incorrectly formatted line in file " + commands[1] + ": " + line);
          continue;
        }
        String title = bookTraits[0];
        String author = bookTraits[1];
        title = title.trim();
        author = author.trim();
        if (title.length() == 0 || author.length() == 0) {// title or author incorrect
          System.out.println(
              "Error: Found incorrectly formatted line in file " + commands[1] + ": " + line);
          continue;
        }
  ...
  public void parseRunLoginAsSubscriber(String[] commands, Scanner scanner) throws ParseException {
    this.checkCommandArgumentsCount(commands, 3);// checks if commands stores 3 arguments
    int cardBarCode = this.parseCardBarCode(commands[1], 1);// check the card bar code
  ...
  public void readProcessLibrarianCommand(Scanner scanner) {
    // TODO This method should catch any ParseException or InstantiationException that may be thrown
    ...
    while (!(commands[0].toUpperCase().equals("9") && commands.length == 1)) { // "9": Exit the
      ...
      try {
        ...
        // methods must be handled using try catch blocks within those methods.
        String error;
        System.out.println(e.getMessage()); // display the exception's error message
